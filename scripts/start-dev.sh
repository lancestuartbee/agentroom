...
ensure_dev_service_worker_reset() {
    [ "$PROD_WEB" = false ] || return 0
    [ "${ENABLE_PWA_IN_DEV:-0}" != "1" ] || return 0

    local sw_file="$PROJECT_DIR/packages/web/public/sw.js"
    mkdir -p "$(dirname "$sw_file")"
    cat > "$sw_file" <<'EOF'
/* Clowder AI dev-only Service Worker reset. Generated by scripts/start-dev.sh. */
...
EOF
    echo -e "${GREEN}  ✓ 开发模式 Service Worker reset 已准备${NC}"
}
