...
export function buildConciergePromptLines(config: ConciergeConfig, _threadId?: string): string[] {
  ...
  return [
    ...
    '- concierge_relay — 向目标 thread 转发用户消息（含目标成员句柄）',
    ...
    '',
    '**分诊四条路径（intent 识别）：**',
    '1. **relay**（传话）— 用户想让你传达信息给某位成员/某个 thread → 生成 relay 确认卡',
    ...
    '**目标成员**: [relay 且用户明确指定成员时填写 @catId；不确定就不要输出 triage-plan，先追问]',
    '**原文**: [用户原话引用]',
    '**操作**: [具体要做什么的一句话说明]',
    '<!-- /triage-plan -->',
    '```',
    ...
    '1. 用户显式 @ 了某位成员 → 直接用',
    '2. 目标 thread 有活跃参与成员 → 作为候选',
    '3. feat_index 显示归属成员 → 作为候选',
    '4. 候选不唯一或为空 → 不要生成 triage-plan，先问用户要传给哪位成员（fail-closed，不盲投）',
    '',
    '**目标引用纪律：**',
